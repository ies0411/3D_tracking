...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", type=str, default="nuScenes", help="KITTI, nuScenes"
    )
    parser.add_argument(
        "--num_hypo", type=int, default=1, help="number of hypothesis used"
    )
...
def conf_thresholding(data_dir, save_dir, score_threshold, num_hypo):
    ...
    for hypo_index in range(num_hypo):
        ...
        for seq_file in seq_list:
            ...
            for frame, frame_data in seq_data.items():
                for id_tmp, obj in frame_data.items():
                    ...
                    # read object score
                    if id_tmp not in trk_id_score.keys():
                        trk_id_score[id_tmp] = list()
                    trk_id_score[id_tmp].append(obj.s)

                    # read object category
                    if id_tmp not in cat_dict.keys():
                        cat_dict[id_tmp] = obj.type
        ...
        save_dir_tmp = os.path.join(save_dir, "data_%d" % (hypo_index))
        ...
        for seq_dir in seq_dir_list:
            ...
            for frame in frame_list:
                ...
                frame_file_save = os.path.join(save_frame_dir, frame_index + ".txt")
