...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="pointrcnn",
        help="we provide pointrcnn on KITTI, megvii for nuScenes",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="H1",
        help="additional string of the folder to be combined",
    )
    parser.add_argument(
        "--num_hypo", type=int, default=1, help="number of hypothesis to combine"
    )
...
def combine_trk_cat(split, dataset, method, suffix, num_hypo):
    ...
    log = os.path.join(
        root_dir, "%s_%s_%s" % (method, split, suffix), "combine_log.txt"
    )
    ...
    for hypo_index in range(num_hypo):
        ...
        save_root = os.path.join(
            root_dir, "%s_%s_%s" % (method, split, suffix), "data" + data_suffix
        )
        ...
        for seq_tmp in seq_list:
            ...
            for subset_tmp in subset:
                file_tmp = os.path.join(
                    root_dir, subset_tmp, "data" + data_suffix, seq_tmp + ".txt"
                )
        ...
        save_root = os.path.join(
            root_dir, "%s_%s_%s" % (method, split, suffix), "trk_withid" + data_suffix
        )
        for seq_tmp in seq_list:
