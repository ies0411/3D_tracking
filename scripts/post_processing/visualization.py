...
def parse_args():
    ...
    parser.add_argument(
        "--result_sha",
        type=str,
        default="pointrcnn_Car_test_thres",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="the index of hypothesis results for visualization",
    )
    parser.add_argument(
        "--highlight_file", type=str, default=None, help="error to be highlighted"
    )
...
def vis(args):
    ...
    if args.highlight_file is not None:
        hl_data_dict = load_highlight(args.highlight_file)
    else:
        hl_data_dict = None
    ...
    if not is_path_exists(trk_root):
        print_log(
            "full %s dataset does not exist at %s, fall back to mini dataset for a quick demo"
            % (args.dataset, trk_root),
            log=log,
        )
        trk_root = os.path.join(
            file_path, "../../data/%s/mini" % args.dataset, subfolder
        )
        assert is_path_exists(trk_root), (
            "error, unfortunately mini data is missing at %s as well" % trk_root
        )
        ...
        # elif args.dataset == 'nuScenes':
            ...
            assert False, "mini data does not support for %s-%s" % (
                args.dataset,
                args.split,
            )
    ...
    for seq in seq_eval:
        ...
        result_dir = os.path.join(
            result_root, "trk_withid_%d/%s" % (args.hypo_index_vis, seq)
        )
        ...
        print_log(
            "seq %s, number of images to visualize is %d" % (seq, num_images), log=log
        )
        start_count = 0
        for count in range(start_count, num_images):
            ...
            result_tmp = os.path.join(
                result_dir, "%06d.txt" % image_index
            )  # load the result
            ...
            print_log(
                "processing index: %d, %d/%d, results from %s"
            ...
            if data_hl is not None and image_index in data_hl.keys():
                id_hl = data_hl[image_index]
            else:
                id_hl = None
            ...
            for object_tmp in object_res:
                ...
                if obj_type not in det_id2str.values():
                    continue
                if hasattr(object_tmp, "score"):
                    if object_tmp.score < score_threshold[obj_type]:
                        continue
            ...
            vis_image_with_obj(
                image_tmp, filtered, [], calib_tmp, hw, save_path=save_path, id_hl=id_hl
            )
            print_log(
                "number of objects to plot is %d" % (len(filtered)),
                log=log,
                display=False,
            )
            count += 1
